...
import { Users, PlusCircle, Search, Pencil, Trash2, Cake, Mail, Phone } from "lucide-react";
import {
  SECTOR_OPTIONS,
  ROLE_OPTIONS,
  sectorLabel,
...
export default function EmployeesList() {
  ...
  const filtered = (employees ?? []).filter((e) => {
    ...
    if (search) {
      ...
      const hay = `${e.fullName} ${e.email ?? ""} ${e.phone ?? ""} ${e.cpf ?? ""}`.toLowerCase();
  ...
  const handleDelete = (id: number) => {
    deleteMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: "Colaborador removido", description: "Cadastro excluído com sucesso." });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Cadastro e gestão da equipe</p>
          </div>
        </div>
      ...
      <Card className="shadow-sm">
        <CardContent className="p-4 grid gap-3 sm:grid-cols-3">
          ...
          <Select value={sectorFilter} onValueChange={setSectorFilter}>
            <SelectTrigger className="h-11"><SelectValue placeholder="Setor" /></SelectTrigger>
            <SelectContent>
          ...
          <CardContent className="p-10 text-center">
            ...
            <Link href="/colaboradores/novo">
              <Button variant="outline" className="mt-4">
                <PlusCircle className="h-4 w-4 mr-2" /> Cadastrar primeiro colaborador
              </Button>
            </Link>
          </CardContent>
        </Card>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <CardTitle className="text-base truncate">{e.fullName}</CardTitle>
              ...
              <CardContent className="space-y-2 text-sm pt-0">
                ...
                {e.birthDate && (
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{shortBirthday(e.birthDate)}</span>
                  </div>
                )}
                <div className="flex gap-2 pt-2 border-t mt-3">
                  <Link href={`/colaboradores/${e.id}/editar`} className="flex-1">
